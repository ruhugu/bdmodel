!       Retrieved from:
!       http://ergodic.ugr.es/cphys/UTILES/dranxor2new.f

	subroutine dranini(iseed0)
...
c
	dseed=iseed0
        ...
        ix(i)=0
        do 200 j=0,nbit-1
        if(randxx(dseed).lt.0.5) ix(i)=ibset(ix(i),j)
200	continue
	ic=0
c
...
!         end
! 
! 	subroutine dranread2(iunit,cadena)
! 	implicit double precision(a-h,o-z)
...
!         end
! 
!         subroutine dranwrite2(iunit,cadena)
! 	implicit double precision(a-h,o-z)
! 	character*100 cadena
!     	parameter(ip=1279)
! 	parameter(np=14)
...
! 	common /icc/ ic
! 	common /gg/ g
! 	open(UNIT=iunit,FILE=cadena,STATUS="UNKNOWN")
!         write (iunit,*) ic
!         write (iunit,*) (ix(i),i=1,ip)
! 	write (iunit,*) (g(i),i=0,m)
! 	close(iunit)
...
!         end
	...
	i=ishft(ix(ic),-np1)
	i2=iand(ix(ic),nn)
	drang=i2*g(i+1)+(nn1-i2)*g(i)
	return
	end


	subroutine dranbm(x1,x2)
	implicit double precision(a-h,o-z)
    	parameter(ip=1279)
	parameter(iq=418)
	parameter(is=ip-iq)
...
21	ix(i)=iv(n+ip-ic+i)
	...
	return
        ...
        function randxx(dseed)
        double precision a,c,xm,rm,dseed,randxx
